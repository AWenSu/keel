...
| D14 | Repo has `.learned/` → rulebook hits injected as a Known pitfalls block into every implementer/reviewer brief and every lens brief; no `.learned/` is disclosed, never silently skipped | `keel-execute` universal rules, `keel-plan-review` Step 2 | same two sections; `keel-execute` INLINE step 2 re-lifts it (the 2026-09-11 audit found INLINE re-lifted every other gate but this one) | `26` |
| D15 | Fix-loop round 4 switches two things at once: the reviewer brief to the verdict-matrix form (every outstanding finding + NEW-defect/regression/test-adequacy answered per-area) and the fixer to `keel-exec-fixer-critical` | `keel-execute` per-task loop step 4 (fix loop) | same | `26` |
