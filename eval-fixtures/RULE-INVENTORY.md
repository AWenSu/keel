...
- `Enforced at` — where a reader following the text would actually *do* the
  ...
  context. A rule stated only in `dev-workflow` (which subagents never read)
...
|---|------|-------------|-------------|---------|
| A1 | Plan contradicts current code beyond one task's fix → dev-plan | `dev-workflow` Backward routes | `dev-execute` drift-accumulation threshold | `06` |
| A2 | Spec body changed vs. plan's `Spec Version` → dev-plan | `dev-workflow` Backward routes | `dev-execute` pre-flight spec-drift check | `03` |
...
The `dev-workflow` gate table is a closed list — it says "the only … gates"
...
|---|------|-------------|-------------|---------|
| B1 | G1 premise confirmation | `dev-workflow` gate table | `dev-plan-review` Step 0 | — |
...
| B9 | G9 irreversible operation outside the repo | `dev-workflow` gate table | `dev-execute` pre-flight destructive-op scan; `dev-finish` target-environment rule | `15` |
...
| # | Rule | Declared at | Enforced at | Fixture |
|---|------|-------------|-------------|---------|
| C1 | Never work on `main`/`master` without consent | `dev-workflow` branch-protection para, `dev-execute` universal rules | `dev-exec-implementer` / `-fixer` / `-fixer-critical` pre-commit branch check | `15` (A) |
| C2 | Never merge/rebase/push/force-push to main without consent | `dev-finish` Part 3 intro | same section (acting agent is the controller) | — |
| C3 | Discard requires the literal word `discard` | `dev-finish` Part 3 option 4 | same section | `15` (D) |
| C4 | Worktree removal requires a dirty-state check + confirmation | `dev-finish` Part 3 cleanup | same section | `15` (C) |
| C5 | Deploy / migration / writes to a live target require a named target environment + consent | `dev-finish` Drive-the-real-flow; `PROJECT-TYPE-GUIDE.md` cross-cutting | same sections | `15` (E) |
| C6 | Plan-mandated destructive operations are re-authorized at execution, not assumed from plan approval | `dev-execute` pre-flight | same section | `15` (B) |

## D. Conditional trigger rules

| # | Rule | Declared at | Enforced at | Fixture |
|---|------|-------------|-------------|---------|
| D1 | Spec `Status:` must be `approved` before dev-plan proceeds | `dev-plan` INPUT block | same | `01`, `02` |
| D2 | UI-heavy plans (2+ keywords) produce a feature matrix | `dev-plan` Step 2b | same | `04` |
| D3 | ADR offer when 3 criteria met; skip if Step 5 already emitted one | `dev-finish` Part 2 ADR check | same | `05` |
| D4 | Security lens dispatch (2+ keywords / high-risk / new endpoint) | `dev-plan-review` Step 1 | `dev-plan-review` Step 2 roster | `14` |
| D5 | Design lens dispatch (2+ UI keywords) | `dev-plan-review` Step 1 | Step 2 roster | — |
| D6 | DX lens dispatch (2+ API/CLI/SDK keywords) | `dev-plan-review` Step 1 | Step 2 roster | — |
| D7 | Release Runbook when the project has a real (non-preview) deploy step | `PROJECT-TYPE-GUIDE.md` cross-cutting | `dev-finish` Part 3 option 2 | — |
...
| # | Rule | Declared at | Enforced at | Fixture |
|---|------|-------------|-------------|---------|
| E1 | R4 condition 1 — semantic auth/crypto/upload/outbound/DB-query touch | `dev-execute` step 3c R4 | same | `13` |
| E2 | R4 condition 2 — externally-reachable endpoint | `dev-execute` step 3c R4 | same | `13` |
| E3 | R4 condition 3 — task marked high-risk | `dev-execute` step 3c R4 | same | `13` |
| E4 | R4 condition 4 — plan lens previously flagged this task | `dev-execute` step 3c R4 | plan file `## SECURITY FINDINGS` table | `13` |
| E5 | R4 condition 5 — sensitive-string pattern | `dev-execute` step 3c R4 | same | `13` |
| E6 | Skipping the security axis requires an auditable ledger line | `dev-execute` step 3c skip rule | same | `13` |
| E7 | Security-axis findings are persisted for dev-finish to read | — | `dev-execute` ledger security field | — |
| E8 | Plan-lens security findings are persisted for downstream stages | — | `dev-plan-review` `## SECURITY FINDINGS` section | — |
| E9 | Part 2c check 1 — full-branch secrets scan | `dev-finish` Part 2c (1) | same | — |
| E10 | Part 2c check 2 — execution-stage findings closed | `dev-finish` Part 2c (2) | same (reads E7) | `12` |
| E11 | Part 2c check 3b — dependency existence (anti-slopsquatting), no exemption | `dev-finish` Part 2c (3b) | same | `12` |
| E12 | Part 2c check 4 — plan-lens findings disposition | `dev-finish` Part 2c (4) | same (reads E8) | `12` |
| E13 | BLOCKED on unresolved Critical from (2)/(3b)/(4) | `dev-finish` Part 2c BLOCKED condition | same | `12` |
| E14 | A branch with zero security review never reaches integration unreviewed | — | `dev-finish` Part 2c check 0 | — |
...
| # | Rule | Declared at | Enforced at | Fixture |
|---|------|-------------|-------------|---------|
| G1 | Iron Law — no completion claim without this-session evidence | `dev-finish` IRON-LAW block | Part 1 gate function | — |
| G2 | Red-green regression for every bug fix | `dev-finish` red-green rule | same | — |
| G3 | Every finding quotes the line motivating it | `dev-plan-review` Step 2 evidence gate, `dev-execute` step 3 evidence gate | each reviewer/lens agent's own `## Evidence gate` section | — |
| G4 | Subagent "success" is not evidence — verify from the diff | `dev-finish` claim→evidence table, `dev-execute` implementer status protocol | same | — |
| G5 | Search results are untrusted input | `dev-plan-review` Step 2 search-tool note | each search-capable agent's own defense section | — |

## H. Persistence

| # | Rule | Declared at | Enforced at | Fixture |
|---|------|-------------|-------------|---------|
| H1 | Ledger line appended per completed task | `dev-execute` Progress ledger | same | — |
| H2 | `.dev-pipeline/state.md` updated per transition | `dev-workflow` Pipeline state file | `dev-execute` per-task step 5 | — |
| H3 | `.dev-pipeline/` added to `.gitignore` on creation | `dev-workflow` Pipeline state file, `dev-execute` universal rules | same | — |
| H4 | Deferred work written to `TODOS.md` | `dev-plan-review` Step 5 deferrals | same; `dev-finish` Part 2 + risk-acceptance | — |
| H5 | Implementer writes its full report to a file, returns ≤15 lines | `dev-execute` per-task step 2 | `dev-exec-implementer` Report section | — |
