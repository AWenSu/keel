...
**Rule source:** `skills/dev-execute/SKILL.md` Per-task loop, step 4 (Fix
...
`dev-exec-reviewer-quality` finds that the task's own brief text
...
The controller does not dispatch `dev-exec-fixer` for this finding and does
...
`dev-exec-reviewer-quality` finds a genuine bug (off-by-one in a loop
bound) with no relationship to the plan's text.

## Expected B

Goes straight to the normal fix loop — `dev-exec-fixer` — no user question,
no G4 gate.
