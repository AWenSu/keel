...
**Architecture:**
```
dev-plan          dev-plan-review              dev-execute                    dev-finish
  (標記已有)   →   + dev-plan-lens-security  →  + dev-exec-reviewer-security → + 資安退場閘門
                    (STRIDE 設計期威脅建模)      (R4 條件觸發，第三審查軸)      (R6 新鮮證據)
```
新增兩個 agent（`dev-plan-lens-security`、`dev-exec-reviewer-security`），
...
- `dev-plan-lens-security` 明確不做程式碼層級審查（該職責屬
  `dev-exec-reviewer-security`）
- `dev-exec-reviewer-security` **獨立成軸，不併入 quality reviewer**
  （兩軸不得合併排名的既有設計哲學）
- `dev-exec-reviewer-security` **read-only**，finding 由 fixer 修正
- R4 觸發條件（任一命中）：`Files:` 觸及認證/授權/session/加密/上傳/外部呼叫/
  資料庫查詢建構路徑；diff 新增或修改對外端點；task 被 dev-plan 標記高風險；
  計畫階段 security lens 對該 task 留有 finding；diff 命中敏感字串樣式
  （`password`/`secret`/`token`/`api[_-]?key`/`BEGIN.*PRIVATE KEY`/連線字串樣式）
- `dev-exec-reviewer-security` 建議釘 **opus**（不同於 spec/quality 的 sonnet）
- 保留 `~/.claude/agents/security-auditor.md` 不動；只移除
  `dev-plan-review/SKILL.md` 中對它的散文提及
- 不實作掃描工具本身；不做滲透測試自動化；不改 `code-reviewer`；不改
  `dev-debug`；不引入外部資安服務或付費 API

**Success Criteria（抄自 spec §六）：**
- [ ] 含登入與檔案上傳的測試計畫跑 `dev-plan-review`，security lens 自動啟用
      且產出至少一則帶 file:line 證據的 finding，分數列入 REVIEW REPORT
- [ ] 硬寫 API Key 的測試 task 跑 `dev-execute`，security 軸回傳
      `VERDICT: FAIL` 並指出該行
- [ ] 純 CSS 調整的測試 task，security 軸不觸發，ledger 記錄跳過理由
- [ ] 存在未關閉 Critical 資安 finding 時，`dev-finish` 回傳 BLOCKED
- [ ] `dev-plan-review/SKILL.md` 對 `security-auditor` 的散文提及已移除，
      無兩套機制並存
...
既有測試框架 —— dev-plan 模板的「write failing test / run / implement /
...
## Task 1: 新增 dev-plan-lens-security agent

**Delivers:** 存在一個 `dev-plan-lens-security` agent，職責為設計期 STRIDE
...
**Files:** `/Users/wen/.claude/agents/dev-plan-lens-security.md`（新檔，
比照 `dev-plan-lens-eng.md`、`dev-plan-lens-dx.md` 的 frontmatter/章節結構）
**Depends on:** none
**Skills:** none
**Interfaces:**
  - Consumes: 無（讀 plan file + spec，同其他 lens）
  - Produces: `SCORE:`/`FINDINGS:`/`APPENDIX:`/`CHECKED:` 輸出格式，供
    Task 2 的 dev-plan-review 派工邏輯與 Step 5 決策分類消費

- [ ] 撰寫 agent frontmatter：`name: dev-plan-lens-security`、description
...
- [ ] 撰寫 Evidence gate 段落（比照 `dev-plan-lens-eng.md` 逐字）
- [ ] 撰寫 Output 格式區塊：`SCORE:`/`FINDINGS:`（含證據/信心/建議編輯）/
      `APPENDIX:`/`CHECKED:`，比照既有 lens
- [ ] 明確寫一句「不做程式碼層級審查，該職責屬 dev-exec-reviewer-security」
- [ ] Output 格式的 `FINDINGS:` 段落要求：每則 finding 的建議編輯須標註
      ...
      dev-execute 端無從判斷某個 task 是否曾被此 lens 標記過
      （dev-plan-review 審查通過的 REVIEW REPORT 是聚合層級摘要，
      無 per-task 結構）
- [ ] `grep -c "STRIDE"` 與 `grep -c "^model: opus"` 確認寫入且各恰好一次
- [ ] Commit（`feat: add dev-plan-lens-security agent — STRIDE threat modeling lens`）

## Task 2: 將 security lens 接入 dev-plan-review

**Delivers:** 對一份命中 2+ 資安詞彙（或高風險標記，或新增對外端點）的計畫
跑 `dev-plan-review`，security lens 被自動選中、順序在 Eng 之後 DX 之前、
跳過時會在 roster 公告中說明理由、分數會出現在最終 REVIEW REPORT。
`security-auditor` 的散文提及已從檔案中完全移除。
**Files:** `/Users/wen/.claude/skills/dev-plan-review/SKILL.md`
（Step 1 第 42-45 行、Step 2 dispatch table 第 57-67 行、Step 2 lens
briefs 段落）
**Depends on:** Task 1
**Skills:** none
**Interfaces:**
  - Consumes: Task 1 的 `dev-plan-lens-security` agent 名稱與輸出格式
  - Produces: 更新後的 lens dispatch table，供 Task 6（dev-workflow roster
...
- [ ] Step 2 dispatch table 新增一列：
      `| Security | dev-plan-lens-security | 資安詞彙 2+／高風險標記／新增對外端點 |`，
      置於 Eng 之後、DX 之前
...
- [ ] `grep -c "security-auditor"` 確認結果為 0（散文提及完全移除）；
      `grep -n "dev-plan-lens-security"` 確認 dispatch table 與 brief
      兩處都有
- [ ] Commit（`feat: wire dev-plan-lens-security into dev-plan-review Step 1/2`）

## Task 3: 新增 dev-exec-reviewer-security agent

**Delivers:** 存在一個 `dev-exec-reviewer-security` agent，read-only，給它
一段含硬寫 API Key 的 diff，回傳 `VERDICT: FAIL` 並指出該行；給它一段純
CSS 調整 diff（理論上不會被派工，但邏輯本身要能正確判斷），回傳
`VERDICT: PASS` 或空 finding。
**Files:** `/Users/wen/.claude/agents/dev-exec-reviewer-security.md`
（新檔，比照 `dev-exec-reviewer-quality.md`、`dev-exec-reviewer-spec.md`
的 frontmatter/章節結構）
**Depends on:** none
**Skills:** none
**Interfaces:**
  - Consumes: 無（讀 diff + task brief，同其他 reviewer）
  - Produces: `AXIS: security` 輸出格式，供 Task 4 的 dev-execute 第三軸
    接線與 fix loop 消費

- [ ] 撰寫 agent frontmatter：`name: dev-exec-reviewer-security`、description
...
- [ ] 撰寫 Prompt Defense Baseline（逐字比照 `dev-exec-reviewer-quality.md`）
- [ ] 撰寫「Diff base」段落（逐字比照兩個既有 reviewer：base commit 明確
      給出、never `HEAD~1`、無 base 則 BLOCKED、排除 `.dev-pipeline/`）
...
- [ ] Commit（`feat: add dev-exec-reviewer-security agent — third review axis`）

## Task 4: 將 security 軸接入 dev-execute 第三審查軸

**Delivers:** `dev-execute` 每個 task 的「Review the diff with two
independent reviewers」步驟，在 R4 五條觸發條件任一命中時，額外派出
`dev-exec-reviewer-security` 為第三軸；不觸發時 ledger 明確記錄
`security axis skipped — <理由>`；三軸輸出彼此不得合併排名；model
matrix 新增一列釘 opus；provenance 記錄本次來源。
**Files:** `/Users/wen/.claude/skills/dev-execute/SKILL.md`（Step 3 第
103-133 行、Model selection table 第 182-196 行、provenance frontmatter）
**Depends on:** Task 3
**Skills:** none
**Interfaces:**
  - Consumes: Task 3 的 `dev-exec-reviewer-security` agent 名稱、輸出格式、
    model 釘選
  - Produces: 更新後的 per-task 審查步驟與 model matrix，供 Task 6
    （dev-workflow roster）與 Task 7（README）引用

- [ ] Step 3 標題「Review the diff with two independent reviewers」改為
      「Review the diff — spec/quality always, security when triggered」
- [ ] Step 3 新增 (c) `dev-exec-reviewer-security` 段落：R4 五條觸發條件
      逐字列出（`Files:` 觸及認證/授權/session/加密/上傳/外部呼叫/DB查詢
      建構路徑；diff 新增或修改對外端點；task 被 dev-plan 標高風險；計畫
...
- [ ] 觸發時的 ledger 條目格式沿用 `.dev-pipeline/progress.md` 既有的
...
- [ ] 第一條件（`Files:` 觸及...路徑）判定方式明訂為語意判斷，不是純
      檔名關鍵字比對：依 dev-plan 既有規則「Delivers 是真相、Files 是
...
- [ ] 第四條件（計畫階段 security lens 對該 task 留有 finding）判定方式
      ...
      關閉」——`dev-plan-review` 的 exit gate 要求 `NO UNRESOLVED
      DECISIONS` 才能進入 dev-execute，若照「未關閉」字面理解，此條件
      在進入 dev-execute 時必然恆為假
- [ ] 新增第六條件：diff 新增相依套件清單中原本不存在的套件名稱（新增
      條目，非版本升級）。但這條不是為了防漏判——純套件清單變更（無
      auth/upload/敏感字串 命中）交由 dev-finish Part 2c(3) 把關即可，
...
- [ ] Model selection table 新增一列：
      `| dev-exec-reviewer-security | opus | R4 觸發時的第三審查軸 — 漏判成本不對稱 |`
- [ ] provenance frontmatter 補一行來源（同 Task 2 格式，指向需求書
      R3/R4/R5）
- [ ] `grep -c "dev-exec-reviewer-security"` 確認 Step 3、Model table 兩處
...
- [ ] Commit（`feat: wire dev-exec-reviewer-security as conditional third axis in dev-execute`）

## Task 5: dev-finish 資安退場閘門

**Delivers:** `dev-finish` 的 Part 2（Success criteria check）之後新增
資安檢查小節，存在未關閉的 Critical 資安 finding 時整個 dev-finish
回傳 BLOCKED，不得宣告完成；secrets 掃描工具不存在時明確標示未執行，
不靜默略過。
**Files:** `/Users/wen/.claude/skills/dev-finish/SKILL.md`（Part 2 與
Part 2b 之間插入新小節；provenance frontmatter）
**Depends on:** Task 4
**Skills:** none
**Interfaces:**
  - Consumes: Task 4 的 security 軸 finding 記錄位置
    （`.dev-pipeline/progress.md` 的 skip/finding 記法）、Task 2 的
...
- [ ] 新增「Part 2c: 資安退場檢查」小節，四項檢查（逐字對應需求書 R6）：
      ...
      需付費 API」，硬性要求安裝 OSS 掃描器會在本機環境下讓 dev-finish
...
- [ ] 明訂 BLOCKED 條件：存在未關閉 Critical 資安 finding 且無使用者
      接受風險決策 → dev-finish 回傳 `BLOCKED: 資安 finding 未關閉`，
...
- [ ] Part 2c 產生的「使用者接受風險」決策，寫入格式沿用既有
      `dev-plan-review` 的 `TODOS.md` 條目慣例（What / Why deferred /
      Effort / Priority，此處額外加 finding 的 file:line），不新開一套
      `.dev-pipeline/progress.md` 格式——decider 在此單人互動式 pipeline
...
- [ ] Part 3（分支整合）既有的 `code-reviewer` 派工提示詞，補一句告知
      ...
      **只改 `dev-finish/SKILL.md` 裡呼叫 `code-reviewer` 時給的 context，
...
- [ ] Commit（`feat: add security exit gate to dev-finish (Part 2c)`）

## Task 6: dev-workflow roster 同步 + security-auditor 舊敘述清理

**Delivers:** `dev-workflow/SKILL.md` 的 subagent roster 表新增兩列
（`dev-plan-lens-security`、`dev-exec-reviewer-security`），與 Task 1/3
的實際 model 釘選一致；同表既有 `security-auditor` 一列的 Stage/Role
欄位更新，不再暗示它涵蓋 stage 3 計畫審查（該職責已移交
`dev-plan-lens-security`）。**不修改 `~/.claude/CLAUDE.md`**——實查該檔
Custom Agents 路由表只收錄使用者手動路由的 9 個 agent，現有 9 個
`dev-*` pipeline 內部 agent（`dev-plan-lens-ceo/eng/design/dx` 等）
一個都不在其中，因為它們是 pipeline 內部派工、使用者從不手動叫。兩個新
agent 屬同一類，比照既有慣例不入全域路由表；roster 記於
`dev-workflow/SKILL.md` 已足夠可查。
**Files:** `/Users/wen/.claude/skills/dev-workflow/SKILL.md`（第 82-101
...
- [ ] `dev-workflow/SKILL.md` roster 表新增：
      `| dev-plan-lens-security | 3 review | STRIDE 威脅建模（設計期） | opus |`
      置於 `dev-plan-lens-dx` 之後、`dev-plan-skeptic` 之前（依 stage 3
      執行順序排列，同表既有慣例）
- [ ] 同表新增：
      `| dev-exec-reviewer-security | 4 execute | 第三審查軸（R4 條件觸發） | opus |`
      置於 `dev-exec-reviewer-quality` 之後
- [ ] 同表既有 `security-auditor` 一列（第 98 行，現寫
      ...
      欄位註明「非 dev-plan-review/dev-execute 自動派工」）——這是舊機制
      被新機制取代後留下的殘留敘述，不清理會讓 roster 表本身出現
      「以為有涵蓋」的假象，恰是本案要消除的那類問題
- [ ] `grep -c "dev-plan-lens-security"` 與
      `grep -c "dev-exec-reviewer-security"` 確認各恰好一次
- [ ] Commit（`docs: sync security agents into dev-workflow roster; clarify security-auditor's non-pipeline scope`）

## Task 7: README 雙語同步 + 全案自我複查

**Delivers:** `README.md`／`README.zh-TW.md` 的 provenance 表、roster 表、
gate 說明，反映兩個新 agent 與 dev-finish 新增的資安閘門；spec 軸與
quality 軸的既有段落逐字比對確認未被誤改。
**Files:** `/Users/wen/myProject/unified-dev-skills/README.md`、
`/Users/wen/myProject/unified-dev-skills/README.zh-TW.md`；並將 Task 1-6
在 `~/.claude/agents/` 與 `~/.claude/skills/` 的變更同步複製進本 repo
對應路徑（`agents/dev-plan-lens-security.md`、
`agents/dev-exec-reviewer-security.md`、
`skills/dev-plan-review/SKILL.md`、`skills/dev-execute/SKILL.md`、
`skills/dev-finish/SKILL.md`、`skills/dev-workflow/SKILL.md`）
...
- [ ] 自我複查一輪：`grep -c "reviewer-spec\|reviewer-quality"` 在
      `dev-execute/SKILL.md` 確認出現次數與變更前相同（既有兩軸行為與
...
- Placeholder 掃描：全文無 TBD／「執行時再定」／「Task N 同上」／未定義
  型別 ✓
- Type-consistency：`dev-plan-lens-security`、`dev-exec-reviewer-security`
...
- 觸及檔案數（15 個：2 新 agent + 5 skill + 1 CLAUDE.md-adjacent roster
  表 + 2 README + 6 個 repo 內對應副本 + TODOS.md）超過 dev-plan Exit
  段「>8 files」的送審門檻，本身即是「此計畫須走 dev-plan-review」的
...
Task 2 (接線 dev-plan-review) — Task 1
Task 3 (新 reviewer agent)   — none
Task 4 (接線 dev-execute)     — Task 3
Task 5 (dev-finish 閘門)      — Task 4
Task 6 (dev-workflow roster 同步)— Task 1, Task 3
...
- SEC-2（WEAKENED）：新增 R4 第六觸發條件（新增相依套件），但僅作 ledger 歸屬標註（指向 dev-finish Part 2c(3)），不另開 opus 呼叫
- SEC-3（WEAKENED，與 SEC-1 同址合併）：dev-finish Part 2c 拆分 (3a)CVE掃描／(3b)套件存在性驗證兩部分，(3b) 無「未執行」豁免、必須實跑；Part 2c 決策改沿用既有 TODOS.md 格式，不新開 persistence 格式
- Eng-2：`dev-execute/SKILL.md:115` 殘留「two independent reviewers」措辭同步改寫
...
- CEO-6：Task 5 補一句，dev-finish 呼叫既有 `code-reviewer` 時附上「哪些 task 已跑過 security 軸」context，聚焦跨 task 組合風險；只改 dev-finish 端 context，不改 code-reviewer 本身，不牴觸 Global Constraints
