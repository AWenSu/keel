# Plan: dev-workflow SDD 元素整合（ADR 提前／UAT 顯式化／Contract Test／功能矩陣／API-heavy 與部署類型預設）
...
`unified-dev-skills`：6 項既有缺口強化（R1-R6）＋ SDD 三項具體機制
（spec 溯源、結構化驗收、核准閘門——既有 dev-plan 只有 spec-first 精神，
缺這三個機制本體）。
...
dev-workflow                              PROJECT-TYPE-GUIDE.md
 + Backward route: spec 改動不符 plan       + API-heavy 型 contract-first 細節(R5)
   記錄版本 → 退回 dev-plan(SDD)             + 部署型 Release Runbook 細節(R6)
```
...
- [ ] `dev-plan-review/SKILL.md` Step 5 含 ADR 產出邏輯，dev-finish Part 2
      標註「若已於 Step 5 處理則略過」
- [ ] `dev-finish/SKILL.md` Part 2 含使用者當場核對 Success Criteria 的
      顯式步驟
- [ ] `dev-exec-reviewer-spec.md` 含 Interfaces block 一致性檢查項，工具
      存在時使用、不存在時退回純比對並誠實回報
- [ ] `dev-plan/SKILL.md` 含功能矩陣產出規則，觸發判準沿用既有 UI 關鍵字
      機制
...
- [ ] spec 未核准（`Status: draft`）時 `dev-plan` 回傳 BLOCKED 退回
      dev-discover；spec 核准後 Success Criteria 採 Given-When-Then 格式；
      plan 執行期間 spec 版本與 plan 記錄不符時，`dev-workflow` Backward
      routes 命中退回 dev-plan
...
明文排除 SDD 強化（「SDD 精神本身已由 dev-plan 覆蓋，不另立項」）。
dev-plan-review Step 0 premise 確認時，使用者明確答覆「不覆蓋，但要增加
其優點」，並於後續澄清問題答覆「三項都要，合併成一個 task」——這是
Task 6 存在的直接授權來源，優先於需求書原文。

**Alternatives considered（dev-discover 慣例，補記於此因本計畫走
lightweight 路徑未經完整 dev-discover）：**
...
測試框架 —— dev-plan 模板的「write failing test / run / implement / run /
...
## Task 1: ADR 決策點提前至 dev-plan-review Step 5   [Risk: Low]

**Delivers:** `dev-plan-review` Step 5 處理 Taste/User Challenge 決策時，
若某決策符合既有判準（難逆＋反直覺＋真 trade-off），當場產出一段 ADR 並
寫入 plan file（或 `docs/adr/` 若存在），而非等到 dev-finish 才第一次被
提及。既有的「Mechanical 自動套用、Taste/Challenge 問使用者」流程本體不變。
**Files:** `/Users/wen/.claude/skills/dev-plan-review/SKILL.md:220-270`
...
**Interfaces:**
  - Consumes: 無（沿用 Step 5 既有 Taste/User Challenge 分類與既有 ADR
    判準——難逆＋反直覺＋真 trade-off，這判準文字已存在於 `dev-finish`
    Part 2，逐字抄用不重寫）
  - Produces: ADR 產出時機標記（「此決策已於 Step 5 產出 ADR」），供
    Task 2 的 dev-finish Part 2 判斷是否要 skip

- [ ] 在 `dev-plan-review/SKILL.md` Step 5，Round 逐項處理迴圈中新增規則：
      每個決策套用完成後，立即判斷是否符合 ADR 判準（難逆＋反直覺＋真
      trade-off——逐字抄用 `dev-finish/SKILL.md:92` 現有那句判準，不重寫
      措辭），符合則追加一段 ADR（Context/Decision/Consequence 三段，
      比照 `dev-finish` Part 2 現有格式）寫入 plan file 的新增
      `## ADR: <決策名>` 小節
- [ ] 明確寫出：不符合判準的決策不產出 ADR，這是既有機制的位置提前，不是
      新增一道強制關卡
- [ ] 附註一句：`dev-discover/SKILL.md:170-176` 既有 ADR offer 判準用逗號
      分隔三條件、`dev-finish/SKILL.md:91-92` 用「+」分隔——同一套判準的
      既有措辭分歧（早於本計畫存在），本 task 逐字抄用 dev-finish 的版本，
      不藉此機會統一三處寫法（Eng lens 標記為既有問題，非本輪範圍）
- [ ] `grep -c "^## ADR:" skills/dev-plan-review/SKILL.md` 確認範例／規則
      文字存在且只出現一次（防重複寫入）
...
## Task 2: dev-finish Part 2 — ADR skip-note ＋ UAT 顯式核對   [Risk: Low]

**Delivers:** `dev-finish` Part 2 的既有 ADR offer 邏輯旁註記「若
...
**Files:** `/Users/wen/.claude/skills/dev-finish/SKILL.md:80-93`（Part 2
...
- [ ] 在 `dev-finish/SKILL.md` Part 2 現有「Same check for ADRs」那句後
      加一句：若該決策已於 `dev-plan-review` Step 5 產出 ADR（沿用 Task 1
      訂出的措辭判斷），此處略過，不重複產出
...
- [ ] `grep -c "使用者當場\|user confirms\|UAT" skills/dev-finish/SKILL.md`
      確認新文字寫入且未重複
...
**Delivers:** `dev-exec-reviewer-spec` 審查一個消費前置 task `Interfaces:`
...
執行過。
**Files:** `/Users/wen/.claude/agents/dev-exec-reviewer-spec.md:30-45`（「The
...
- [ ] `grep -c "contract test\|spectral\|pact" agents/dev-exec-reviewer-spec.md`
      確認新文字寫入且未重複
...
## Task 4: dev-plan 功能矩陣產出規則   [Risk: Low]

**Delivers:** `dev-plan` 寫 plan header 時（Step 2），若計畫命中既有
UI/多角色關鍵字判準（沿用 `dev-plan-lens-design` 既有的 2+
...
事後抓（`dev-plan-lens-design`）提前到規劃階段寫的當下。
**Files:** `/Users/wen/.claude/skills/dev-plan/SKILL.md:85-87`（Step 2
...
**Skills:** none
**Interfaces:**
  - Consumes: `dev-plan-lens-design.md` 現有的觸發關鍵字清單（讀取比照，
    不重新定義；若該 agent 檔案的關鍵字清單未來變動，此規則自動沿用同一
    來源，不在 dev-plan 內重複硬寫關鍵字列表——用一句話引用該 agent 檔案
    路徑，而非複製關鍵字字面值）
  - Produces: `## Feature Matrix` 小節格式，供 `dev-plan-lens-design`
    Step 3 審查時作為既有「狀態矩陣」輸出的比對基準（該 agent 既有輸出
    已有「狀態矩陣」欄位，本任務讓其有規劃期產出物可比對，而非審查時
    憑空重新枚舉）
- [ ] 在 `dev-plan/SKILL.md` Step 2 之後新增 Step 2b「功能矩陣（條件式）」
      小節：命中判準時（明確寫「與 `dev-plan-lens-design.md` frontmatter
...
- [ ] 明確寫出：未命中判準的計畫完全略過此步驟，不加重非 UI 專案的規劃
      負擔
- [ ] `grep -c "^## Feature Matrix\|Step 2b" skills/dev-plan/SKILL.md`
      確認新文字寫入且未重複
...
型別新增 contract-first 細節：計畫涉及對外或服務間 API 時，dev-plan 階段
...
計畫，其餘型別完全不受影響。
**Files:** `/Users/wen/myProject/unified-dev-skills/PROJECT-TYPE-GUIDE.md`
...
- [ ] Commit（`docs: add contract-first + Release Runbook details to
      PROJECT-TYPE-GUIDE.md`，本 repo `unified-dev-skills`——此檔本身即
      repo 內檔案，非 live-copy，不需 Task 7 補同步）

## Task 6: SDD 三項機制強化（spec 溯源／結構化驗收／核准閘門）   [Risk: Med]

**Delivers:** dev-plan 現有 spec-first 精神補上 SDD 的三個具體機制：
(a) spec 為單一真相源——spec header 新增顯式 `Status: draft|approved` 欄位，
`dev-plan` 的 INPUT contract 檢查此欄位為 `approved` 才可進入，非
approved 時 `BLOCKED: spec 未核准 → 退回 dev-discover`（複用既有
`BLOCKED:` 格式，非新造一套錯誤格式）；spec 核准後若又被修改，`dev-plan`
header 記錄核准時的 spec 版本（commit hash 或修改時間戳），`dev-execute`
既有的「plan 與 code 矛盾則退回 dev-plan」路徑旁新增一條「spec 本體在
執行期間被改到與 plan 記錄的版本不符 → 同樣退回 dev-plan 重新對齊」，
複用 `dev-workflow` 既有 Backward routes 表格的既有列格式，不新造一張表；
(b) Success Criteria 改用結構化驗收格式——`dev-discover` 的 spec 模板與
`dev-plan` 的 plan header 模板，Success Criteria 欄位改寫成 Given-When-Then
三段式（場景／條件／預期結果），取代現行自由 prose checklist，讓每條標準
可直接對應成一個可驗證的斷言而非一句話宣稱。
**Files:** `/Users/wen/.claude/skills/dev-discover/SKILL.md:159-166`
（Write spec 段落，spec 檔案模板處）、
`/Users/wen/.claude/skills/dev-plan/SKILL.md:19-32, 69-85`（INPUT block
...
`/Users/wen/.claude/skills/dev-workflow/SKILL.md:30-38`（Backward routes
表格，新增一列）
**Depends on:** Task 4（同時編輯 `dev-plan/SKILL.md`，序列化避免平行編輯
...
- [ ] 在 `dev-discover/SKILL.md` 的 spec 檔案模板，header 加一行
...
- [ ] 在 `dev-plan/SKILL.md` 的 INPUT block（檔案開頭 `INPUT` 那行）補一句：
      **若 INPUT 走 spec 路徑**，該 spec 的 `Status:` 欄位須為 `approved`，
      ...
      非改寫成散文
- [ ] 在 `dev-plan/SKILL.md` Step 2 的 plan header 模板新增一行
      `**Spec Version:** <spec 核准時的 commit hash 或時間戳，抄自 spec
      的 Status 欄位>`
- [ ] 在 `dev-workflow/SKILL.md` 的 Backward routes 表格新增一列（比照
      既有列格式）：`| Spec 本體在執行期間被改到與 plan 記錄的
      Spec Version 不符 | dev-execute | dev-plan |`
- [ ] `grep -c "Status: draft\|approved" skills/dev-discover/SKILL.md
      skills/dev-plan/SKILL.md`、`grep -c "Given-When-Then\|Spec Version"
      skills/dev-plan/SKILL.md` 確認新文字寫入且未重複
- [ ] 手動走查：假想一份 spec 標記 `Status: draft`，核對 dev-plan INPUT
...
## Task 7: 雙語 README 同步 ＋ dev-finish Part 3 Release Runbook 接線   [Risk: Low]

**Delivers:** `README.md`／`README.zh-TW.md` 反映 Task 1-6 的七項變更（不
新增 roster 列——本輪皆為既有 skill/agent 的接線強化，非新增 agent）；
`dev-finish` Part 3 的 Push+PR 選項後接上 Task 5 定義的 Release Runbook
產出邏輯（讀取 `PROJECT-TYPE-GUIDE.md` 判準決定是否觸發）；provenance
表格新增本次需求書一列。
**Files:** `/Users/wen/myProject/unified-dev-skills/README.md`（Provenance
表格，約第 288-300 行附近）、`README.zh-TW.md`（對應表格）、
`/Users/wen/.claude/skills/dev-finish/SKILL.md:179-192`（Part 3，選項 2
「Push + PR」之後）
...
- [ ] `README.md`／`README.zh-TW.md` 找到描述 dev-plan-review Step 5、
      dev-finish Part 2、dev-exec-reviewer-spec、dev-plan、dev-discover、
      dev-workflow、PROJECT-TYPE-GUIDE 的既有段落，逐一補上本輪七項變更的
      一句話摘要（比照既有段落的簡潔程度，不展開成教學文）
- [ ] Provenance 表格新增一列，比照既有格式：
      `| dev-workflow SDD 元素整合需求書 (2026-08-07) | internal doc, not a
      repo | sources: 外部分享的 SDD/Contract-first/ADR 流程比對 |`（兩份
      README 各一列，中文版用中文欄位敘述比照既有中文列的措辭）
- [ ] 在 `dev-finish/SKILL.md` Part 3「Push + PR」選項條目之後新增一句：
      若專案命中 `PROJECT-TYPE-GUIDE.md` 的「有正式部署環節」判準
      （Task 5 訂出的措辭），追加產出 Release Runbook 小節寫入 PR body，
      否則略過
- [ ] `grep -c "Release Runbook" skills/dev-finish/SKILL.md` 確認寫入且
      未重複；`diff` 兩份 README 對應段落確認中英文版描述的七項變更
      一致（非逐字翻譯，但涵蓋範圍相同）
- [ ] 本輪共動 9 個檔案（7 個 skill/agent + 2 份 README），超過
      dev-workflow 自身「>8 files → review」門檻——已正確走本次
      dev-plan-review，此處補一道 Eng lens 建議的交叉檢查：對每個
...
- [ ] 在 `TODOS.md` 新增一筆延後項（What/Why deferred/Effort/Priority
      ...
      `.dev-pipeline/eval-fixtures/` 假想計畫測試集；Effort: M；
      Priority: Low（非本輪阻斷項，供未來需要迴歸驗證此類變更時使用）
- [ ] 手動走查：Task 1-6 七項 Success Criteria 逐條對照 README 新增文字，
      確認每一項都在 README 有對應一句話提及，無遺漏
- [ ] 將 Task 1-6 全部改動檔案（`dev-plan-review/SKILL.md`、
      `dev-finish/SKILL.md`、`dev-exec-reviewer-spec.md`、
      `dev-plan/SKILL.md`、`PROJECT-TYPE-GUIDE.md`、`dev-discover/SKILL.md`、
      `dev-workflow/SKILL.md`）複製進本 repo 對應路徑（比照前次 `13ee5ee`
      ...
      最新版，需補齊）
- [ ] Commit（`docs: sync READMEs and repo copies for dev-workflow SDD
      integration`，本 repo `unified-dev-skills`；已核實 `~/.claude` 不是
...
- [Mechanical] Task 6 INPUT gate checklist 措辭消歧義（明寫「若 INPUT 走
  ...
  補強
- [Mechanical] Task 1 附註 ADR 判準既有措辭分歧（dev-discover 逗號 vs
  dev-finish 加號），標記為既有問題非本輪範圍
- [Mechanical] Task 7 新增 diff 位元組級交叉檢查（CEO lens 指出僅
  `grep -c` 不足以驗證整檔複製正確）
- [Mechanical] Task 7 新增 TODOS.md 延後項——eval-fixture harness（Eng lens
  Low finding，非本輪阻斷項）
- [Mechanical] Task 1-6 全部 Commit 步驟修正——dev-execute 前置檢查發現
...
- [User Challenge, REFUTED] Eng lens Critical：Task 6 INPUT gate 會擋既有
  無 spec 檔路徑（Regression Iron Rule）——`dev-plan-skeptic-critical`
  核對後 REFUTED：計畫文字是「補一句」非「改寫 INPUT」，且 Medium
  shortcut 根本不經過 dev-plan 這個 stage，無法建構具體失效情境
- [User Challenge, REFUTED] CEO lens High：ADR 三處機制衝突——
  `dev-plan-skeptic-critical` 核對後 REFUTED：三處觸發的是不同時間點的
  不同決策（spec 階段 vs plan 審查階段 vs 收尾階段），dev-plan 既有
  「不得重新論戰 ADR」規則與 Task 2 的 skip-note 已三重堵死重複產出
- [User Challenge, REFUTED] CEO lens High：Task 6(a) 核准閘門價值被高估——
  `dev-plan-skeptic-critical` 核對後 REFUTED：既有機制是同一 session 內
