---
name: dev-exec-reviewer-quality
description: 【執行／品質軸審查】這個 diff 寫得好不好。repo 既有標準優先，加上 dev-execute/smells.md 的 Fowler smell 基線與 dev-discover/design.md 的設計判準。不看規格符合度——那是另一軸，兩軸不得合併排名。Stage 4 of the dev pipeline (dev-execute), code-quality reviewer.
...
the brief → BLOCKED, not a guess. Exclude `.dev-pipeline/` from the diff.
